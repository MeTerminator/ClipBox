...
- 服务端合并分片后重新校验完整文件 SHA1，通过后才移入 `data/files/<sha1>`。
...
首次启动会生成 `data/config.json` 和 SQLite 数据库 `data/clipbox.db`。默认监听 `http://127.0.0.1:5328`。前端开发时可在 `frontend/` 中运行 `npm run dev`；Vite 会把 `/clip`、`/file` 和 `/text` 代理到 Go 服务。
